          ...
          {expertisePillars.map((pillar, i) => (
            ...
            >
              <div className="grid gap-5 sm:grid-cols-[3.5rem_1fr] sm:gap-8">
                ...
                <ParallaxLayer speed={cardParallax[i] ?? 10}>
                  <div className="rounded-[var(--radius-lg)] border border-(--color-glass-border) bg-(--color-glass) p-6 backdrop-blur-sm transition-colors duration-300 hover:border-(--color-accent)/40 sm:p-8">
                    <div className="flex flex-wrap items-baseline justify-between gap-2">
                      <div className="flex items-center gap-3">
                        ...
                        <h3 className="text-xl font-bold text-(--color-on-band) sm:text-2xl">{pillar.name}</h3>
                      </div>
                    ...
                    <p className="mt-4 inline-flex max-w-xl rounded-[var(--radius-pill)] border border-(--color-accent)/30 bg-(--color-accent)/10 px-3 py-1.5 text-sm font-medium text-(--color-accent)">
                      {pillar.plain}
                    </p>
                    <p className="mt-4 max-w-3xl text-sm leading-relaxed text-(--color-on-band-dim) sm:text-base">
                      {pillar.description}
                    </p>
                  </div>
                </ParallaxLayer>
